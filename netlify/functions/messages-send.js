...
// Accepts: (lead_id) OR (contact_id) OR (user_id + to), plus templateKey and optional provider_message_id.
// Enforces: template enabled per user, subscribed=true, wallet >= 1¢, dedupe by provider_message_id.
// Logs: full row into public.messages (direction='outgoing'), then RPC debit 1¢ on success.
  ...
  try {
    ...
    if (lead_id) {
      const { data: l, error: lerr } = await db
        .from("leads")
        .select("id,user_id,name,phone,state,beneficiary,beneficiary_name,status,sold")
        .eq("id", lead_id)
        .maybeSingle();
    ...
    // Load user templates + enabled flags
    ...
    const { data: w, error: werr } = await db.from("user_wallets").select("balance_cents").eq("user_id", user_id).maybeSingle();
    ...
    // Insert full message row (so it shows in Messages UI)
    const insertRow = {
      user_id,
    ...
    try {
      ...
      // Optional: mark row billing_error. Skipping to keep logic simple.
    }
