...
export default function AdminConsole() {
  ...
  async function saveRow(row) {
    ...
    setErr("");
    try {
      // Seats -> team_seat_counts (upsert by team_id)
      if (row.team_id != null) {
      ...
      // Balance -> user_wallets (upsert by user_id)
    ...
    <div className="space-y-4 p-4 text-white">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button onClick={load} disabled={fetching} className="rounded-lg border border-white/20 px-3 py-1.5 text-sm hover:bg-white/10">
            {fetching ? "Refreshing…" : "Refresh"}
          </button>
          <button onClick={saveAll} disabled={saving} className="rounded-lg border border-emerald-400/30 px-3 py-1.5 text-sm hover:bg-emerald-400/10">
            ...
            {rows.map((r) => {
              ...
              return (
                <tr key={r.id} className="border-t border-white/10">
                  ...
                  <td className="px-3 py-2">
                    <button onClick={() => saveRow(r)} className="rounded-md border border-white/20 px-3 py-1 hover:bg-white/10">
                      Save
                    </button>
                  </td>
                </tr>
