...
function pickByWindow(slotLabels, slotHours, windowWord) {
  ...
  if (/afternoon|tarde/i.test(windowWord)) {
    const a = pairs.filter(p => p.hour >= 12 && p < 17); // safeguard typo fix below
    const a2 = pairs.filter(p => p.hour >= 12 && p.hour < 17);
    return (a2.length ? a2 : slotLabels).map(p => p.label);
  }
