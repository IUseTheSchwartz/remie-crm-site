...
/* Enabled map defaults: ALL OFF */
const DEFAULT_ENABLED = Object.fromEntries(
  TEMPLATE_DEFS.map((t) => [t.key, false])
);
...
export default function MessagingSettings() {
  ...
  // Templates (text bodies)
  const [templates, setTemplates] = useState(() => ({ ...DEFAULTS }));
  const [saveState, setSaveState] = useState("idle");
  const saveTimer = useRef(null);

  // Enabled map (per-template) — persisted to message_templates.enabled
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Enabled flags: prefer enabled column; default all OFF
      const initialEnabled =
        tmpl?.enabled && typeof tmpl.enabled === "object"
          ? { ...DEFAULT_ENABLED, ...tmpl.enabled }
          : { ...DEFAULT_ENABLED };
    ...
    <div className="space-y-6">
      ...
      <header className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
        ...
        <p className="mt-1 text-sm text-white/70">
          Manage your text balance and message templates. (Twilio connection can
          be finished later.)
        </p>
      </header>
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-col gap-2 md:items-end">
            ...
            <div className="flex items-center gap-2">
              ...
              {customMsg && (
                <div className="text-xs text-amber-300">{customMsg}</div>
              )}
            </div>
      ...
      <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
        <div className="mb-3 flex items-center gap-2">
          ...
          <div className="text-xs text-white/60">
            ...
            {saveState === "error" && (
              <span className="ml-3 text-rose-300">Save failed</span>
            )}
          </div>
        </div>
            ...
            return (
              ...
              >
                ...
                <textarea
                  ...
                  rows={5}
                  className={[
                    "w-full rounded-lg border border-white/15 bg-white/5 px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-indigo-400/50",
                    enabled ? "" : "opacity-70",
                  ].join(" ")}
        ...
        {cheatOpen && (
          ...
          >
            ...
            <p className="mb-3 text-xs text-white/70">
              Paste these tokens into any template. The system replaces them
            ...
            <div className="space-y-3 text-xs">
              ...
              <VarRow
                token="military_branch"
                desc="Military branch (if provided)"
              />{" "}
              {/* 🆕 */}
      ...
      <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
        ...
        <ul className="mt-2 list-disc space-y-1 pl-5 text-xs text-white/70">
          <li>
            Only text clients who have opted in or have an existing
            relationship.
          </li>
