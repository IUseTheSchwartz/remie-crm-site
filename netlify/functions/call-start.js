...
// AGENT-FIRST: call the AGENT first; webhook will dial the LEAD on human answer, then bridge.
// Caller ID selection priority shown to the *lead* (for the second leg):
...
/**
 * --- US NPA → USPS two-letter state map ---
 */
const NPA_STATE = {
  205:"AL",251:"AL",256:"AL",334:"AL",938:"AL", 907:"AK",
...
/** Decide the best caller ID (the number the **lead** will see). */
...
exports.handler = async (event) => {
  ...
  const agent_number = String(body.agent_number || "").trim(); // agent cell (we call this FIRST)
  const lead_number  = String(body.lead_number  || "").trim(); // target (we call this AFTER agent answers)
  ...
  // Lead NPA (area code) for picking caller ID (shown to lead)
  ...
  // Pack context for webhook (this is the AGENT leg)
  const client_state = b64({
    kind: "crm_outbound_agent_leg",   // 👈 AGENT-FIRST flow
    user_id: agent_id,
    contact_id,
    // Lead info for the next leg the webhook will place:
    lead_number,
    // The number we want showing to the LEAD when we dial them:
    from_number: callerId,
    lead_npa: leadNPA,
    lead_state: stateFromAreaCode(leadNPA),
    // Optional media to play to the LEAD while the agent is ringing:
    ringback_url: RINGBACK_URL || null,
    // Webhook should dial the lead with AMD in "continue" mode, then:
    //  - if HUMAN: bridge agent<->lead and log answered_by="human"
    //  - if MACHINE: still allow agent to leave VM; log answered_by="machine"
    b_timeout_secs: 25,
    record: recordFlag
  });

  // Create the AGENT call first
  const createPayload = {
    to: agent_number,
    from: callerId, // any owned number is fine; not shown to lead
