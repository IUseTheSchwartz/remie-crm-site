...
export async function handler(event) {
  try {
    ...
    // Create (or reuse) a customer by email; attach user_id in metadata
    // (Stripe will reuse existing customer with same email if you pass that id.)
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      // Optional: trial periods can be set on the price itself
    });
