...
exports.handler = async (event) => {
  try {
    ...
    const orderRes = await fetch("https://api.telnyx.com/v2/number_orders", {
      ...
      body: JSON.stringify({
        phone_numbers: [{ phone_number_id: telnyx_phone_id }],
      }),
    });
    ...
    // 2) Assign messaging profile on the number
    ...
    if (!patchRes.ok) {
      return json({ error: "telnyx_assign_profile_failed", detail: patchJson }, 502);
    }

    // 3) Save in DB
    const db = getServiceClient();

    // upsert-friendly: try insert; on conflict(e164) do update
