...
exports.handler = async (event) => {
  ...
  if (!TELNYX_CONNECTION_ID) return bad(500, "TELNYX_CONNECTION_ID is not set");

  let body = {};
  try { body = JSON.parse(event.body || "{}"); } catch { return bad(400, "Invalid JSON"); }

  const agent_id     = String(body.agent_id || body.user_id || "").trim();
  const agent_number = String(body.agent_number || "").trim();
  const lead_number  = String(body.lead_number  || "").trim();
  ...
  // choose best caller ID (match NPA if possible)
